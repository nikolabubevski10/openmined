...
const createLinks = (
  ...
  return links
    ...
    .map(({ type, title, auth, unauth, ...link }: LinkProps) => {
      // TODO:SSR should handle cloneElement in sever side
      // if (type === 'element' && typeof window !== 'undefined')
      if (type === 'element')
        return React.cloneElement(link.element, { key: title });

      const as: any = link.to ? { as: LinkWrapper } : {};
...
export default ({ noScrolling }) => {
  ...
  // TODO:SSR - implement nextjs version window scrolling
  const scrollY = 0;
